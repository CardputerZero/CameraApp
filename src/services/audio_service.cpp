...
#if !USE_DESKTOP && CAMERA_APP_USE_ALSA
...
bool load_wav_asset(const char* asset_name, WavData& wav)
{
    ...
    if (!found_fmt || !found_data || audio_format != 1 ||
        wav.channels == 0 || wav.sample_rate == 0 ||
                      ...
                      unsigned int channels)
{
    ...
    while (written < frames) {
        const snd_pcm_sframes_t rc = snd_pcm_writei(pcm,
...
bool playback_wav(const WavData& wav)
{
    ...
    const bool ok = write_pcm_frames(pcm,
                                     reinterpret_cast<const int16_t*>(wav.pcm.data()),
                                     frames,
                                     wav.channels);
    snd_pcm_drain(pcm);
    return ok;
...
struct AudioService::Impl {
    ...
    bool play_asset(const char* asset_name)
    {
        ...
        playback_thread = std::thread([wav = std::move(wav)]() {
            (void)playback_wav(wav);
        });
        return true;
#endif
    }
    ...
    bool play_shutter()
    {
        return play_asset(kShutterAsset);
    }

    bool play_click()
    {
        return play_asset(kPressAsset);
    }
    ...
    bool start_recording(const std::string& path)
    {
        ...
        record_ok = true;
        recording = true;
        record_thread = std::thread([this, path]() {
            capture_loop(recording, record_ok, path);
        });
