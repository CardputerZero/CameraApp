...
void request_program_exit()
{
    g_exit_requested = 1;
}

void handle_program_signal(int)
{
...
lv_display_t* setup_display()
{
    ...
    const char* fb_device = env_fb_device && env_fb_device[0] ? env_fb_device :
                            env_launcher_fb_device && env_launcher_fb_device[0] ? env_launcher_fb_device :
    ...
    const char* drm_device = env_device && env_device[0] ? env_device : "/dev/dri/card0";
...
void handle_navigation(screen::ScreenManager& manager,
                       app::AppStateMachine& state_machine)
{
    ...
    if (camera_vm && camera_view) {
        ...
        if (capture_state == service::CaptureState::Saved || capture_state == service::CaptureState::Failed) {
