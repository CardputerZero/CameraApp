...
{
    if (freeze_preview_until_ms_ && static_cast<int32_t>(lv_tick_get() - freeze_preview_until_ms_) < 0) {
        return;
    }
...
void CameraView::set_capture_status(service::CaptureState state, const std::string& path)
{
    ...
    capture_status_hide_timer_ = lv_timer_create(capture_status_hide_timer_cb,
                                                 kCaptureStatusVisibleMs,
                                                 this);
