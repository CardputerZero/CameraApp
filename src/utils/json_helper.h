...
class JsonValue {
 public:
  ...
  const char* as_string(const char* def = "") const {
    return (node_ && cJSON_IsString(node_) && node_->valuestring) ? node_->valuestring : def;
  }
...
class Json {
 public:
  ...
  bool save_file(const std::string& path, bool formatted = true) const;
  ...
  JsonValue operator[](const char* key) { return JsonValue(cJSON_GetObjectItem(root_, key)); }

  JsonValue operator[](int index) { return JsonValue(cJSON_GetArrayItem(root_, index)); }

  bool valid() { return root_ != nullptr; }

  void reset(cJSON* root = nullptr) {
    clear();
    root_ = root;
  }
  ...
  static Json object();
  static Json array();
  static cJSON* string(const char* value);
  static cJSON* number(double value);
  static cJSON* boolean(bool value);
  static cJSON* null();
  bool add(const char* key, cJSON* item);
  bool append(cJSON* item);
