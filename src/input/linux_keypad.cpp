...
{
    constexpr size_t kKeyBitsSize = (KEY_MAX + sizeof(unsigned long) * 8) / (sizeof(unsigned long) * 8);
    std::array<unsigned long, kKeyBitsSize> key_bits{};
    if (::ioctl(fd, EVIOCGBIT(EV_KEY, sizeof(key_bits)), key_bits.data()) < 0) {
        return false;
    }

    return test_bit_(key_bits, KEY_ESC) ||
           test_bit_(key_bits, KEY_ENTER) ||
           test_bit_(key_bits, KEY_KPENTER) ||
           test_bit_(key_bits, KEY_F) ||
           test_bit_(key_bits, KEY_X) ||
           test_bit_(key_bits, KEY_Z) ||
           test_bit_(key_bits, KEY_C) ||
           test_bit_(key_bits, KEY_1) ||
           test_bit_(key_bits, KEY_4) ||
           test_bit_(key_bits, KEY_5) ||
           test_bit_(key_bits, KEY_6) ||
           test_bit_(key_bits, KEY_7) ||
...
bool LinuxKeypad::open_default()
{
    ...
    if (!opened) {
        LOG_WARN("No Linux keyboard input device opened. Set CAMERA_APP_KEYBOARD_DEVICE=/dev/input/eventX if needed.");
    }
...
bool LinuxKeypad::open_device(const std::string& path)
{
    ...
    LOG_DEBUG("{} Linux input device: {}",
              grab_input ? "Opened and grabbed" : "Opened shared",
              path);
    return true;
#else
    (void)path;
